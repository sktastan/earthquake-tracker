// ---------------------------------------------------------------------
//      Applies the "Solid Mint" (Light) UI theme styles dynamically.
// ---------------------------------------------------------------------
(function () { // Start of IIFE

    const themeId = 'dynamic-styles-solid-mint';

    // Dynamic CSS Injection for Earthquake Tracker - Solid Mint Theme
    ...
    // Color Palette (Solid Mint)
    const mintBg = '#F0FFF0'; // Honeydew - very light
    const panelBg = 'rgba(224, 255, 224, 0.9)'; // Semi-transparent light mint
    const borderLight = '#C8E6C9';    // Pale Green
    const borderMedium = '#A5D6A7';   // Light Green
    const textHeader = '#1B5E20';   // Dark Green
    const textBody = '#2E7D32';     // Green
    const textMuted = '#4CAF50';     // Medium Green
    const accentPrimary = '#66BB6A';  // Green Accent
    const accentSecondary = '#81C784';// Lighter Green Accent
    const tooltipSolidBg = 'rgba(232, 245, 233, 0.95)';
    // Filter to make (assumed dark) icons appear as accentPrimary (#66BB6A)
    const iconFilterSolid = 'brightness(0) saturate(100%) invert(68%) sepia(17%) saturate(1189%) hue-rotate(76deg) brightness(96%) contrast(86%)';
...
th {
    background-color: rgba(220, 240, 220, 0.7); /* Lighter panelBg for mint */
    color: ${accentPrimary};
    font-weight: 600;
...
.sidebar a:hover {
    color: ${textHeader};
    background-color: rgba(102, 187, 106, 0.2); /* accentPrimary with alpha */
...
.signal-overlay {
    ...
    width: 28px;
    height: 28px;
    background-color: rgba(102, 187, 106, 0.7); /* accentPrimary Signal */
    border-radius: 50%;
    display: none;
    pointer-events: none;
...
.signal-overlay.active {
    display: block;
    animation: pulse-signal-solid-mint 1.4s infinite ease-out;
}

@keyframes pulse-signal-solid-mint {
    0% {
        transform: translate(-50%, -50%) scale(0.9);
        box-shadow: 0 0 4px 2px rgba(102, 187, 106, 0.6);
        opacity: 0.8;
    }
    70% {
        transform: translate(-50%, -50%) scale(1.9);
        box-shadow: 0 0 12px 20px rgba(102, 187, 106, 0);
        opacity: 0;
    }
    100% {
        transform: translate(-50%, -50%) scale(0.9);
        box-shadow: 0 0 4px 2px rgba(102, 187, 106, 0);
        opacity: 0;
    }
}
...
.low-magnitude {
    background-color: rgba(129, 199, 132, 0.15); /* Pale accentSecondary bg */
    border-left: 4px solid ${accentSecondary};
    color: ${textMuted};
}

.moderate-magnitude {
    background-color: rgba(102, 187, 106, 0.15); /* Pale accentPrimary bg */
    border-left: 4px solid ${accentPrimary};
    color: ${textMuted};
}

.high-magnitude {
    background-color: rgba(200, 230, 201, 0.2); /* Pale borderLight bg */
    border-left: 4px solid ${borderLight};
    color: ${textBody};
}

.very-high-magnitude {
    background-color: rgba(165, 214, 167, 0.2); /* Pale borderMedium bg for contrast */
...
.progress-overlay {
    ...
    width: 100%;
    height: 100%;
    background-color: rgba(224, 255, 224, 0.9); /* panelBg transparent */
...
.nav-button:hover {
    background-color: rgba(102, 187, 106, 0.2); /* accentPrimary transparent */
}

.nav-button.active {
    background-color: rgba(129, 199, 132, 0.3); /* accentSecondary transparent */
}
...
.earthquakeInfo, .weatherInfo {
    ...
    max-width: 400px; /* Consistent with chromatic-glass */
    z-index: 10; /* Above map, potentially below sidebar */
    /* Padding, background, border, box-shadow, color will come from the .card class applied in HTML */
    /* margin: 12px 0; is removed as fixed positioning handles placement */
}

.earthquakeInfo {
    top: 80px;
    border-left: 4px solid ${accentPrimary}; /* Mint's accent */
}

.weatherInfo {
    top: 430px; /* Consistent with chromatic-glass, adjust if needed */
    border-left: 4px solid ${accentSecondary}; /* Mint's secondary accent */
}
...
.new-earthquake-row {
    animation: highlight-row-solid-mint 1s ease-in-out; 
}

@keyframes highlight-row-solid-mint {
    0%, 100% {
        background-color: transparent;
        transform: scale(1);
    }
    50% {
        background-color: rgba(102, 187, 106, 0.15); /* accentPrimary with alpha */
...
`;
    ...
    console.log("Solid Mint (Light) theme applied.");
