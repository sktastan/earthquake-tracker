// ---------------------------------------------------------------------
//      Applies the "Solid Peach" (Light) UI theme styles dynamically.
// ---------------------------------------------------------------------
(function () { // Start of IIFE

    const themeId = 'dynamic-styles-solid-peach';

    // Dynamic CSS Injection for Earthquake Tracker - Solid Peach Theme
    ...
    // Color Palette (Solid Peach)
    const peachBg = '#FFF3E0'; // Pale Orange/Peach
    const panelBg = 'rgba(255, 224, 180, 0.9)'; // Semi-transparent light peach
    const borderLight = '#FFCCBC';    // Light Peach
    const borderMedium = '#FFAB91';   // Medium Peach
    const textHeader = '#D84315';   // Deep Orange
    const textBody = '#F4511E';     // Orange Red
    const textMuted = '#FF7043';     // Coral
    const accentPrimary = '#FF8A65';  // Light Coral Accent
    const accentSecondary = '#FFB74D';// Light Orange Accent
    const tooltipSolidBg = 'rgba(255, 236, 217, 0.95)';
    // Filter to make (assumed dark) icons appear as accentPrimary (#FF8A65)
    const iconFilterSolid = 'brightness(0) saturate(100%) invert(65%) sepia(30%) saturate(1303%) hue-rotate(327deg) brightness(102%) contrast(101%)';
...
th {
    background-color: rgba(255, 210, 170, 0.7); /* Lighter panelBg for peach */
    color: ${accentPrimary};
    font-weight: 600;
...
.sidebar a:hover {
    color: ${textHeader};
    background-color: rgba(255, 138, 101, 0.2); /* accentPrimary with alpha */
...
.signal-overlay {
    ...
    width: 28px;
    height: 28px;
    background-color: rgba(255, 138, 101, 0.7); /* accentPrimary Signal */
    border-radius: 50%;
    display: none;
    pointer-events: none;
...
.signal-overlay.active {
    display: block;
    animation: pulse-signal-solid-peach 1.4s infinite ease-out;
}

@keyframes pulse-signal-solid-peach {
    0% {
        transform: translate(-50%, -50%) scale(0.9);
        box-shadow: 0 0 4px 2px rgba(255, 138, 101, 0.6);
        opacity: 0.8;
    }
    70% {
        transform: translate(-50%, -50%) scale(1.9);
        box-shadow: 0 0 12px 20px rgba(255, 138, 101, 0);
        opacity: 0;
    }
    100% {
        transform: translate(-50%, -50%) scale(0.9);
        box-shadow: 0 0 4px 2px rgba(255, 138, 101, 0);
        opacity: 0;
    }
}
...
.low-magnitude {
    background-color: rgba(255, 183, 77, 0.15); /* Pale accentSecondary bg */
    border-left: 4px solid ${accentSecondary};
    color: ${textMuted};
}

.moderate-magnitude {
    background-color: rgba(255, 138, 101, 0.15); /* Pale accentPrimary bg */
    border-left: 4px solid ${accentPrimary};
    color: ${textMuted};
}

.high-magnitude {
    background-color: rgba(255, 204, 188, 0.2); /* Pale borderLight bg */
    border-left: 4px solid ${borderLight};
    color: ${textBody};
}

.very-high-magnitude {
    background-color: rgba(255, 171, 145, 0.2); /* Pale borderMedium bg for contrast */
...
.progress-overlay {
    ...
    width: 100%;
    height: 100%;
    background-color: rgba(255, 224, 180, 0.9); /* panelBg transparent */
...
.nav-button:hover {
    background-color: rgba(255, 138, 101, 0.2); /* accentPrimary transparent */
}

.nav-button.active {
    background-color: rgba(255, 183, 77, 0.3); /* accentSecondary transparent */
}
...
.earthquakeInfo {
    top: 80px;
    border-left: 4px solid ${accentPrimary}; /* Peach's accent */
}

.weatherInfo {
    top: 430px; /* Consistent with chromatic-glass, adjust if needed */
    border-left: 4px solid ${accentSecondary}; /* Peach's secondary accent */
}
...
.new-earthquake-row {
    animation: highlight-row-solid-peach 1s ease-in-out; 
}

@keyframes highlight-row-solid-peach {
    0%, 100% {
        background-color: transparent;
        transform: scale(1);
    }
    50% {
        background-color: rgba(255, 138, 101, 0.15); /* accentPrimary with alpha */
...
@media (max-width: 719px) {
    ...
    .tableContainer.visible th {
        top: 58px; 
        background-color: rgba(255, 224, 180, 0.9); 
    }
...
`;
    ...
    console.log("Solid Peach (Light) theme applied.");
